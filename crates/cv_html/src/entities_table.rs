//! WHATWG named character reference table -- the FULL canonical set.
//!
//! Generated from html.spec.whatwg.org/entities.json (the canonical
//! machine-readable table referenced by the HTML Standard, section 13.5).
//! Do not hand-edit: regenerate with `tools/gen_entities.py`.
//!
//! Each entry key is the reference name WITHOUT the leading ampersand
//! but WITH the trailing semicolon where the spec lists one, so the
//! historical semicolon-optional forms (e.g. `amp` and `amp;`) are
//! distinct entries. Values are the decoded UTF-8 string (1-2 code
//! points). The slice is sorted by key, enabling binary search and the
//! longest-prefix match required by the Named character reference state
//! (HTML Standard section 13.2.5.73).

/// Number of entries in the table (sanity check / docs).
pub const ENTITY_COUNT: usize = 2231;

/// Length in bytes of the longest reference name (without leading `&`,
/// with trailing `;` where present). Bounds the longest-match scan.
pub const MAX_NAME_LEN: usize = 32;

/// The full WHATWG named-character-reference table, sorted by name.
/// `(name, decoded)` where `name` omits the leading ampersand.
