//! AES-128 and AES-256 per FIPS 197.
//!
//! Pure software, table-based, constant-time-leaning. Not the fastest
//! implementation we'll ever ship; M5 swaps in an AES-NI backend behind
//! the same `encrypt_block` interface.

#![allow(clippy::needless_range_loop)]

pub const BLOCK_SIZE: usize = 16;
pub const KEY_128: usize = 16;
pub const KEY_256: usize = 32;

const SBOX: [u8; 256] = [
    ,
];

const RCON: [u8; 11] = [
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
];

fn xtime(b: u8) -> u8 {
    let high = b & 0x80;
    let shifted = b << 1;
    if high != 0 { shifted ^ 0x1b } else { shifted }
}

fn sub_bytes(state: &mut [u8; 16]) {
    for b in state.iter_mut() {
        *b = SBOX[*b as usize];
    }
}

fn shift_rows(state: &mut [u8; 16]) {
    // Row r (0-3) shifts left by r bytes. State indexed column-major.
    let s = *state;
    // Row 1 shift left by 1.
    state[1] = s[5];
    state[5] = s[9];
    state[9] = s[13];
    state[13] = s[1];
    // Row 2 shift left by 2.
    state[2] = s[10];
    state[10] = s[2];
    state[6] = s[14];
    state[14] = s[6];
    // Row 3 shift left by 3.
    state[3] = s[15];
    state[7] = s[3];
    state[11] = s[7];
    state[15] = s[11];
}

fn mix_columns(state: &mut [u8; 16]) {
    for c in 0..4 {
        let a0 = state[c * 4];
        let a1 = state[c * 4 + 1];
        let a2 = state[c * 4 + 2];
        let a3 = state[c * 4 + 3];
        let t = a0 ^ a1 ^ a2 ^ a3;
        state[c * 4] ^= t ^ xtime(a0 ^ a1);
        state[c * 4 + 1] ^= t ^ xtime(a1 ^ a2);
        state[c * 4 + 2] ^= t ^ xtime(a2 ^ a3);
        state[c * 4 + 3] ^= t ^ xtime(a3 ^ a0);
    }
}

fn add_round_key(state: &mut [u8; 16], round_key: &[u8]) {
    for i in 0..16 {
        state[i] ^= round_key[i];
    }
}

#[derive(Debug, Clone)]
pub struct Aes128 {
    round_keys: [u8; 16 * 11],
}

impl Aes128 {
    pub fn new(key: &[u8; KEY_128]) -> Self {
        let mut rk = [0u8; 16 * 11];
        rk[..16].copy_from_slice(key);
        let mut i = 16;
        while i < rk.len() {
            let mut t = [rk[i - 4], rk[i - 3], rk[i - 2], rk[i - 1]];
            if i % 16 == 0 {
                t = [t[1], t[2], t[3], t[0]];
                for b in t.iter_mut() {
                    *b = SBOX[*b as usize];
                }
                t[0] ^= RCON[i / 16];
            }
            for j in 0..4 {
                rk[i + j] = rk[i - 16 + j] ^ t[j];
            }
            i += 4;
        }
        Self { round_keys: rk }
    }

    pub fn encrypt_block(&self, block: &mut [u8; 16]) {
        add_round_key(block, &self.round_keys[..16]);
        for round in 1..10 {
            sub_bytes(block);
            shift_rows(block);
            mix_columns(block);
            add_round_key(block, &self.round_keys[round * 16..(round + 1) * 16]);
        }
        sub_bytes(block);
        shift_rows(block);
        add_round_key(block, &self.round_keys[10 * 16..11 * 16]);
    }
}

#[derive(Debug, Clone)]
pub struct Aes256 {
    round_keys: [u8; 16 * 15],
}

impl Aes256 {
    pub fn new(key: &[u8; KEY_256]) -> Self {
        let mut rk = [0u8; 16 * 15];
        rk[..32].copy_from_slice(key);
        let mut i = 32;
        let n: usize = 8; // words in key
        while i < rk.len() {
            let mut t = [rk[i - 4], rk[i - 3], rk[i - 2], rk[i - 1]];
            let pos = i / 4;
            if pos % n == 0 {
                t = [t[1], t[2], t[3], t[0]];
                for b in t.iter_mut() {
                    *b = SBOX[*b as usize];
                }
                t[0] ^= RCON[pos / n];
            } else if pos % n == 4 {
                for b in t.iter_mut() {
                    *b = SBOX[*b as usize];
                }
            }
            for j in 0..4 {
                rk[i + j] = rk[i - 32 + j] ^ t[j];
            }
            i += 4;
        }
        Self { round_keys: rk }
    }

    pub fn encrypt_block(&self, block: &mut [u8; 16]) {
        add_round_key(block, &self.round_keys[..16]);
        for round in 1..14 {
            sub_bytes(block);
            shift_rows(block);
            mix_columns(block);
            add_round_key(block, &self.round_keys[round * 16..(round + 1) * 16]);
        }
        sub_bytes(block);
        shift_rows(block);
        add_round_key(block, &self.round_keys[14 * 16..15 * 16]);
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    fn unhex(s: &str) -> Vec<u8> {
        (0..s.len())
            .step_by(2)
            .map(|i| u8::from_str_radix(&s[i..i + 2], 16).unwrap())
            .collect()
    }

    fn hex(b: &[u8]) -> String {
        let mut s = String::new();
        for x in b {
            s.push_str(&format!("{x:02x}"));
        }
        s
    }

    /// FIPS 197 Appendix B (AES-128 single-block vector).
    #[test]
    fn fips197_aes128() {
        let key: [u8; 16] = unhex("000102030405060708090a0b0c0d0e0f")
            .try_into()
            .unwrap();
        let pt: [u8; 16] = unhex("00112233445566778899aabbccddeeff")
            .try_into()
            .unwrap();
        let mut block = pt;
        Aes128::new(&key).encrypt_block(&mut block);
        assert_eq!(hex(&block), "69c4e0d86a7b0430d8cdb78070b4c55a");
    }

    /// FIPS 197 Appendix C.3 (AES-256 single-block vector).
    #[test]
    fn fips197_aes256() {
        let key: [u8; 32] =
            unhex("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f")
                .try_into()
                .unwrap();
        let pt: [u8; 16] = unhex("00112233445566778899aabbccddeeff")
            .try_into()
            .unwrap();
        let mut block = pt;
        Aes256::new(&key).encrypt_block(&mut block);
        assert_eq!(hex(&block), "8ea2b7ca516745bfeafc49904b496089");
    }
}
